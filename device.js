...
$.widget("sv.device_rtrslider", $.sv.widget, {
  ...
  //
  // Darstellung der UZSU Einträge und Darstellung Widget in Form eine Liste
	// mit den Einträgen
  // Umsetzung
  ...
  // Basis der Architektur: document.update und document.click baut die
	// handler in die Seite für das Popup ein.
  // document.update kopiert bei einem update die Daten aus dem Backend (per
	// Websocket) in das DOM Element ("uzsu") ein
  // document.click übernimmt die Daten aus dem DOM Element in Variable des JS
	// Bereichs und baut über runtimepopup
  // dynamisch header, body und footer des popup zusammen und hängt sie an die
  ...
  // den aktuellen Stand ab und werden von dort in die Variablen
	// zurückgespeichert, wenn notwendig (save, sort).
  // In der Struktur können Zeilen angehängt (add) oder gelöscht werden (del).
  ...
  // "timeOffsetType":'m' 'm' = Offset in Minuten, '' Offset in Höhengrad
	// (Altitude)
  // "condition" : { Ein Struct für die Verwendung mit conditions (aktuell nur
  ...
  // }
  // "delayedExec": { Ein Struct für die Verwendung mit delayed exec (aktuell
...
$.widget("sv.device_uzsu", $.sv.widget, {
  ...
  _update: function(response) {
    // data-item ist der sh.py item, in dem alle Attribute lagern, die für die
	// Steuerung notwendig ist ist ja vom typ dict. das item, was tatsächlich
	// per
    // Schaltuhr verwendet wird ist nur als attribut (child) enthalten und wird
	// ausschliesslich vom Plugin verwendet. wird für das rückschreiben der
	// Daten an smarthome.py benötigt
    ...
    // Initialisierung zunächst wird festgestellt, ob Item mit Eigenschaft
  ...
  _uzsuFillTableRow: function(responseEntry, tableRow) {
    ...
    if(responseEntry.value != null) {
      // beim Schreiben der Daten Unterscheidung, da sonst das Element falsch
		// genutzt wird mit Flipswitch für die bool Variante
    ...
    // und die pull down Menüs richtig, damit die Einträge wieder stimmen und
	...
	// FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR,SA,SU aus und setze für jeden Eintrag
	// den Button.
  ...
  // Toggelt die eingabemöglichkeit für SUN Elemente in Abhängigkeit der
	// Aktivschaltung
  ...
  _uzsuParseAndCheckResponse: function(response) {
    ...
    // Fehlerbehandlung für ein nicht vorhandenes DOM Objekt. Das response
    ...
    // jetzt kommt noch die Liste von Prüfungen, damit hinterher keine Fehler
	// passieren, zunächst fehlerhafter designType (unbekannt)
    ...
    $.each(response.list, function(numberOfRow, entry) {
      ...
      // wenn designType = '2' und damit fhem auslegung ist muss der JSON
		// String auf die entsprechenden einträge erweitert werden (falls nichts
		// vorhanden)
      if (designType == '2') {
...
$.widget("sv.device_uzsuicon", $.sv.device_uzsu, {
  ...
  _events: {
    'click': function(event) {
      // hier werden die Parameter aus den Attributen herausgenommen und beim
		// Öffnen mit .open(....) an das Popup Objekt übergeben
      // und zwar mit deep copy, damit ich bei cancel die ursprünglichen werte
		// nicht überschrieben habe
      var response = jQuery.extend(true, {}, this._uzsudata);

      if (this._uzsuParseAndCheckResponse(response)) {
        // Öffnen des Popups bei clicken des icons und Ausführung der
		// Eingabefunktion
    ...
    // Aufbau des Footers
      tt += "<div class='uzsuTableFooter'>" +
          "<div class='uzsuRowFooter'>" +
            ...
            tt +=
          "</div>";
    // und der Abschluss des uzsuClear als Rahmen für den float:left und des
	// uzsuPopup divs
    tt += "</div></div>";
    return tt;
  },
  ...
  _uzsuFillTable: function(response) {
    var self = this;
    // Tabelle füllen. Es werden die Daten aus der Variablen response gelesen
	// und in den Status Darstellung der Widgetblöcke zugewiesen. Der aktuelle
	// Status in dann in der Darstellung enthalten !
    var numberOfEntries = response.list.length;
    // jetzt wird die Tabelle befüllt allgemeiner Status, bitte nicht mit attr,
  ...
  _uzsuSaveTable: function(response, saveSmarthome) {
	...
	// zurückgespielt bitte darauf achten, dass das zurückspielen exakt dem der
  ...
  _uzsuAddTableRow: function(response) {
    ...
    // und Daten ausfüllen. hier werden die Zeile wieder mit dem Status
  ...
  _uzsuSortTime: function(response, e) {
    // erst aus dem Widget zurücklesen, sonst kann nicht im Array sortiert
	// werden (alte daten)
    this._uzsuSaveTable(response, false);
  ...
  _uzsuRuntimePopup: function(response) {
    ...
    // dann speichern wir uns für cancel die ursprünglichen im DOM gespeicherten
	...
	// folgenden das Popup initialisiert, geöffnet und die schliessen
...
$.widget("sv.device_uzsutable", $.sv.widget, {
  ...
  // Kohler Start - 20210703
  // Toggelt die eingabemöglichkeit für SERIES Elemente in Abhängigkeit der
	// Aktivschaltung
